 ...
 *      ChangeLog:  1, Release initial version on "03/29/2017 01:58:32 PM"
 *                 
 ********************************************************************************/
...
#define s3c_gpio_read(reg)       __raw_readl((reg)+s3c_gpb_membase)                 //读写操作寄存器
...
static int s3c_hw_init(void)
{
    ...
    {
        release_mem_region(S3C_GPB_BASE, S3C_GPB_LEN);           //主要是检查传入地址的合法性，建立页表（包括访问权限），完成物理地址到虚拟地址的转换
        return -ENOMEM;
    }
...
static int __init s3c_led_init(void)    //初始化函数
{
    int                    result;
    dev_t                  devno;      //保存设备编号
    ...
    if (0 != dev_major) /*  Static */
    {
        ...
        result = register_chrdev_region (devno, dev_count, DEV_NAME);//获得设备编号，成功时为0
    }
    else
    {
        result = alloc_chrdev_region(&devno, dev_minor, dev_count, DEV_NAME);       //动态分配
...
ERROR:
    ...
    cdev_del(led_cdev);                                 //注销
    unregister_chrdev_region(devno, dev_count);        //释放设备编号
    return result;
}

static void __exit s3c_led_exit(void)    //清除函数
{
    dev_t devno = MKDEV(dev_major, dev_minor);          //由主/次设备号构造一个dev_t数据项
